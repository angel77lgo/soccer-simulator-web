...
export function BracketNode({ match, onSave }: BracketNodeProps) {
  ...
  return (
    <div className={`border bg-card transition-colors w-56 ${isFinished ? "border-foreground/15" : "border-border"}`}>
      <div className={`grid grid-cols-[1fr_auto] items-center gap-3 px-3 py-2.5 ${homeWon ? "bg-field/10" : ""}`}>
      ...
      <div className={`grid grid-cols-[1fr_auto] items-center gap-3 border-t border-border px-3 py-2.5 ${awayWon ? "bg-field/10" : ""}`}>
        <div className="flex items-center gap-2 min-w-0">
