...
export function TeamFlag({ code, flagUrl, size = "md", className, alt }: TeamFlagProps) {
  const sizeClasses = {
  ...
  const imageSrc = flagUrl ? getTeamLogoUrl({ fifaCode: code, flagUrl }) : getFlagSvgUrl(code);
